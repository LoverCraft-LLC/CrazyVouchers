            ...
            switch (args[0].toLowerCase()) {
                case "redeem" -> {
                    ...
                    if (hasPermission(sender, "admin"))
                        this.crazyManager.getVoucherCodes().forEach(voucherCode -> completions.add(voucherCode.getCode()));
                }

                case "open" -> {
                    if (hasPermission(sender, "admin"))
                        for (int i = 1; i <= this.inventoryManager.getMaxPages(); i++)
                            completions.add(String.valueOf(i));
                }

                case "give", "giveall" -> {
                    if (hasPermission(sender, "admin"))
                        this.crazyManager.getVouchers().forEach(voucher -> completions.add(voucher.getName()));
                }
            }
